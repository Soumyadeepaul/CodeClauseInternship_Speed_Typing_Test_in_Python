...
def stopTest():
    try:
        #after time exceed writable will be False
        global writeAble
        writeAble = False
...
def restart():
    ...
    #call this function for re taking the test
    resetWritingLabels()

#function for time control
def addSecond():
    if writeAble:
        #decrease the time by 1 sec after every 1000ms
        global passedSeconds
        passedSeconds -= 1
        timeLeftLabel.configure(text=f'{passedSeconds} seconds')
        #below 10 sec remaining change the color
        if passedSeconds<10:
...
def keyPress(event=None):
    try:
        #if the alphabet from keyboard matches with the current alphabet than
        ...
        #if backspace and len of the left side string is greater than 1 than
...
def resetWritingLabels():
    ...
    #when time is there writeAble will be True
    global writeAble
    writeAble=True
    #on pressing keys
    ...
    #after 1 min result will be shown and user will anable to write furthur buy calling stopTest functionality
    window.after(60000,stopTest)
    #after 1 sec call the addSecond functionality
    window.after(1000,addSecond)
#start button functionality
def start():
    #remove the start button
    startButton.destroy()
    #call the functionally of the application
    resetWritingLabels()
